...
export const ContentDetailsModal: React.FC<ContentDetailsModalProps> = ({ 
  ...
  const loadTmdbDetails = async () => {
      const info = await tmdb.getDetailedInfo(content.title, content.contentType);
      if (info) setDetails(info);
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center md:p-4">
      ...
      <div className="relative bg-[#181818] w-full md:max-w-4xl rounded-t-2xl md:rounded-xl shadow-2xl overflow-y-auto max-h-[90vh] md:max-h-none animate-slide-up md:animate-scale-up no-scrollbar">
        ...
        <div className="p-4 md:p-8">
           {/* Metadata Row */}
           <div className="flex flex-wrap items-center gap-3 text-sm text-gray-300 font-semibold mb-6">
               <span className="text-green-400 font-bold">
                   {details?.rating ? `${Math.round(details.rating * 10)}% Match` : '98% Match'}
               </span>
               <span>{getYear(details?.releaseDate)}</span>
               ...
               {details?.genres && (
                   <span className="hidden md:inline text-gray-400 text-xs border border-gray-700 px-2 py-0.5 rounded-full">
                       {details.genres[0]}
                   </span>
               )}
           </div>
           ...
           <div className="grid md:grid-cols-[2fr_1fr] gap-8">
               ...
               <div>
                   ...
                   <div className="flex flex-col md:flex-row gap-3 mb-6">
                      <Button onClick={onPlay} className="w-full md:w-auto py-3 px-8 text-lg font-bold flex items-center justify-center gap-2 bg-white text-black hover:bg-gray-200">
                      ...
                      <Button 
                        ...
                        className={`w-full md:w-auto py-3 px-8 font-bold flex items-center justify-center gap-2 ${isDownloaded ? 'bg-gray-800 text-gray-400' : 'bg-[#2a2a2a] text-white hover:bg-[#333]'}`}
                      >
                          {downloading ? (
                               <>
                   ...
                   <p className="text-white text-sm md:text-base leading-relaxed mb-6">
                     {details?.tagline && <span className="block text-gray-400 italic mb-2">{details.tagline}</span>}
                     {content.description}
                   </p>
               ...
               <div className="text-sm space-y-4">
                   {details?.cast && details.cast.length > 0 && (
                       <div>
                           ...
                           <div className="flex flex-wrap gap-1">
                               {details.cast.map((actor: string, idx: number) => (
                                   <span key={idx} className="text-white hover:underline cursor-pointer">
                                       {actor}{idx < details.cast.length - 1 ? ',' : ''}
                                   </span>
                               ))}
                           </div>
                       </div>
                       ...
                       <div>
                           ...
                           <div className="flex flex-wrap gap-1">
                               {details.genres.map((genre: string, idx: number) => (
                                   <span key={idx} className="text-white hover:underline cursor-pointer">
                                       {genre}{idx < details.genres.length - 1 ? ',' : ''}
                                   </span>
                               ))}
                           </div>
                       </div>
                   ...
                   {details?.director && details.director.length > 0 && (
                       <div>
                           <span className="text-gray-500 block mb-1">{content.contentType === 'movie' ? 'Directed by:' : 'Created by:'}</span>
                           <span className="text-white">{details.director.join(', ')}</span>
                       </div>
                   )}
               </div>
           </div>
           ...
           {content.contentType === 'series' && (
             <div className="border-t border-gray-800 pt-6 mt-6">
                ...
                <div className="space-y-4">
                  {[1, 2, 3].map(ep => (
                    <div key={ep} className="flex items-center gap-4 p-3 hover:bg-[#222] rounded-lg cursor-pointer transition" onClick={onPlay}>
                        <div className="w-32 aspect-video bg-gray-800 rounded relative flex items-center justify-center overflow-hidden flex-shrink-0">
                           <img src={content.thumbnailUrl} className="opacity-60 object-cover w-full h-full" />
